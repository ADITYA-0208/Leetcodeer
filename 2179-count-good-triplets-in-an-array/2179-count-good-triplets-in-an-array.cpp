class Solution {
    ...
    void updateSegTree(ll i,ll start,ll end,ll& query_idx){
        ...
        updateSegTree(2*i,start,mid,query_idx);
        updateSegTree(2*i+1,mid+1,end,query_idx);
        seg_tree[st_idx] = seg_tree[2*st_idx] + seg_tree[2*st_idx+1];
    }
    int rangeSumQuery(ll i,ll start,ll end,ll qs,ll qe){
        if(qs>end or qe<start)  return 0;//Case-1: No overlap
        if(start>=qs and end<=qe)   return seg_tree[i];//Case-2: Total Overlap

        //Case-3: Partial Overlap
        ll mid = start + (end-start)/2;
        ll left_sum = rangeSumQuery(2*i,start,mid,qs,qe);
        ll right_sum = rangeSumQuery(2*i+1,mid+1,end,qs,qe);
